...
// Endpoint API untuk mengecek IP
app.get('/check', async (req, res) => {
    ...
    // Menentukan delay palsu antara 20 - 500 ms untuk respons API
    const fakeDelay = Math.floor(Math.random() * (500 - 20 + 1)) + 20;

    // Pastikan `result.delay` menampilkan nilai dari `fakeDelay`
    result.delay = `${fakeDelay} ms`;

    setTimeout(() => {
        res.setHeader('Content-Type', 'application/json');
        res.send(JSON.stringify(result, null, 4));
    }, fakeDelay); // Menunda respons untuk simulasi delay
});

// Menangani permintaan root ("/")
app.get('/', (req, res) => {
    res.status(200).send("IP Checker API is running. Use /check?ip=<IP_ADDRESS> to check an IP.");
});

// Konfigurasi port untuk server
// Menggunakan process.env.PORT untuk kompatibilitas dengan Vercel atau environment lainnya
